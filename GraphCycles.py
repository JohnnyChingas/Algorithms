"""
Dealing with Graph Cycles:  Implementation of Depth First Search methods
T. Melano
...
def process_edge(x,y):
    ...
    if parent[x] != y and parent[x] != -1:  # We have found a back edge
        print "Parent of x=%d is %d, not %d"%(x,parent[x],y)
        print "Parent of y=%d is %d"%(y,parent[y])
        print "discovered[%d] = %s"%(y,discovered[y])
        print "processed[%d] = %s"%(y,processed[y])
        print "There is a cycle from %d to %d:"%(y,x)
